...
namespace OsteoYoga.Helper
{
    public class Email
    {
        ...
        public virtual void SendForPatientValidation(Date date)
        {
            var toAddress = new MailAddress(date.Contact.Mail, date.Contact.FullName);

            string body = string.Format("<html><head></head><body>Bonjour, <br /><br />Votre demande de rendez-vous a été validée. <br /><br />" + "Resumé du rendez-vous : " + "Date : {0}<br />" + "Horaire : {1} <br />" + "Patient : {2} <br /><br /> Cordialement,<br />Nicolas Truffart</body></html>", date.Day.ToString("dd/MM/yyyy"), date.TimeSlot, date.Contact);
        ...
        public virtual void SendForAdminPropose(Date date)
        {
            var toAddress = new MailAddress(Constants.GetInstance().MailNico, Constants.GetInstance().NomNico);

            string body = string.Format("<html><head></head><body>Bonjour, <br /><br />Une demande de rendez-vous a été proposée.<br /> En attente de validation du patient <br /><br />" + "Resumé du rendez-vous : " + "Date : {0}<br />" + "Horaire : {1} <br />" + "Patient : {2} </body></html>", date.Day.ToString("dd/MM/yyyy"), date.TimeSlot, date.Contact);
